#!/usr/bin/env python3
"""RUG (University of Groningen) timetable viewer - single-file web GUI.

Serves a browser UI on http://127.0.0.1:8000 that talks to the same API used by
https://rooster.rug.nl/current. You build a filter of course + group pairs; for
each course you pick your student group so the schedule shows all lectures but
only your group's tutorials. Courses and groups both autocomplete as you type.

Pure Python standard library - no pip installs needed. Run:
    python3 launch.py
    ...
    def do_POST(self):
        ...
        try:
            if self.path == "/api/search":
                items = search(payload["kind"], payload.get("term", ""))
                return self._json({"items": items})
            if self.path == "/api/course-groups":
                groups = course_groups(payload.get("courseCodes", []))
                return self._json({"groups": groups})
            if self.path == "/api/programme-groups":
            ...
            if self.path == "/api/generate":
                ...
                return self._json({"activities": activities})
...
def main():
    ...
    print(f"RUG schedule UI running at {url}")
